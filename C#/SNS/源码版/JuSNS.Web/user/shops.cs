...
namespace JuSNS.Web.user
{
    public class shops : BasePage
    {
            ...
            else
            {
                ...
                if (userinfo != null)
                {
                    ...
                    else
                    {
                        ...
                        context.Put("cpagetitle", "商品 - " + userinfo.TrueName + "的空间");
        ...
        protected void ShowList(ref VelocityContext context, int userid)
        {
            ...
            DataTable dt = JuSNS.Home.UtilPage.GetGoodsPage(userid.ToString(), string.Empty, 0, uid, PageIndex, recount, out ReCount, out PgCount, st);
            if (PgCount < 1) { PgCount = 1; }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    info.Add("showedit", "<a href=\""+root+"/app/shop/new" + ExName + "?gid=" + dr["id"] + "\" class=\"edit1\" title=\"编辑\"></a>");
                }
                if (isadmin > 0)
                {
