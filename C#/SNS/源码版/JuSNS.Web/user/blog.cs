...
namespace JuSNS.Web.user
{
    public class blog : BasePage
    {
            ...
            else
            {
                ...
                if (userinfo != null)
                {
                    ...
                    else
                    {
                        ...
                        context.Put("cpagetitle", "博客 - " + userinfo.TrueName + "的空间");
        ...
        protected void ShowList(ref VelocityContext context, int uid)
        {
            ...
            DataTable dt = JuSNS.Home.UtilPage.GetBlogPage(uid.ToString(), keys, 0, string.Empty, userid, PageIndex, recount, out ReCount, out PgCount, st);
            if (PgCount < 1) { PgCount = 1; }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                string TitleSTR = Input.FilterHTML(dr["title"]);
