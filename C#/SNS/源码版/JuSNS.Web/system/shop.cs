...
namespace JuSNS.Web.system
{
    public class shop : ManagePage
    {
        ...
        protected void ShowInfo(ref NVelocity.VelocityContext context)
        {
            ...
            context.Put("cpagetitle", "商品管理 - 管理中心");
        ...
        protected void ShowList(ref VelocityContext context)
        {
            ...
            if (!string.IsNullOrEmpty(kwd))
            {
                ...
                st[0].Blur = 3;
                dt = JuSNS.Home.UtilPage.GetPage("manager_goods_key_aspx", PageIndex, recount, out ReCount, out PgCount, st);
            }
            else
            {
                dt = JuSNS.Home.UtilPage.GetPage("manager_goodsall_aspx", PageIndex, recount, out ReCount, out PgCount, null);
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                info.Add("goodsname", Input.GetSubString(goodsname, 50));
                if (Convert.ToBoolean(dr["isrec"]))
                {
                    info.Add("showrec", "<a href=\"javascript:;\" title=\"已推荐\" onclick=\"RecAll(" + dr["id"] + "," + uid + ",0,'goods')\" class=\"showrec1\"></a>");
                }
                else
                {
                    info.Add("showrec", "<a href=\"javascript:;\" title=\"未推荐\" onclick=\"RecAll(" + dr["id"] + "," + uid + ",1,'goods')\" class=\"showrec\"></a>");
                }
                info.Add("state", Public.GetEnumStateOp(dr["IsLock"], Convert.ToInt32(dr["id"]), uid, "goods"));
                info.Add("id", dr["id"]);
                infolist.Add(info);
            }
            dt.Dispose();
