    ...
    public class topic : ManagePage
    {
        ...
        protected void ShowList(ref VelocityContext context)
        {
            ...
            if (!string.IsNullOrEmpty(q))
            {
                if (q == "0")
                {
                    dt = JuSNS.Home.UtilPage.GetPage("manager_topic_parentid0_aspx", PageIndex, recount, out ReCount, out PgCount, null);
                }
                else
                {
                    dt = JuSNS.Home.UtilPage.GetPage("manager_topic_parentid1_aspx", PageIndex, recount, out ReCount, out PgCount, null);
                }
            }
            else
            {
                if (!string.IsNullOrEmpty(kwd))
                {
                    ...
                    st[0].Blur = 3;
                    dt = JuSNS.Home.UtilPage.GetPage("manager_topic_key_aspx", PageIndex, recount, out ReCount, out PgCount, st);
                }
                else
                {
                    dt = JuSNS.Home.UtilPage.GetPage("manager_topicall_aspx", PageIndex, recount, out ReCount, out PgCount, null);
                }
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (Convert.ToInt32(dr["TopicID"]) == 0)
                {
                    if (Convert.ToBoolean(dr["isrec"]))
                    {
                        info.Add("showrec", "<a href=\"javascript:;\" title=\"已推荐\" onclick=\"RecAll(" + dr["id"] + "," + uid + ",0,'topic')\" class=\"showrec1\"></a>");
                    }
                    else
                    {
                        info.Add("showrec", "<a href=\"javascript:;\" title=\"未推荐\" onclick=\"RecAll(" + dr["id"] + "," + uid + ",1,'topic')\" class=\"showrec\"></a>");
                    }
                }
                else
                {
                ...
                info.Add("id", dr["id"]);
                info.Add("state", Public.GetEnumStateOp(dr["IsLock"], Convert.ToInt32(dr["id"]), uid, "topic"));
