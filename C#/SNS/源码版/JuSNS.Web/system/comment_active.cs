...
{
    public class comment_active : ManagePage
    {
        ...
        protected void ShowList(ref VelocityContext context)
        {
            ...
            if (!string.IsNullOrEmpty(kwd))
            {
                ...
                st[0].Blur = 3;
                dt = JuSNS.Home.UtilPage.GetPage("manager_comment_active_key_aspx", PageIndex, recount, out ReCount, out PgCount, st);
            }
            else
            {
                dt = JuSNS.Home.UtilPage.GetPage("manager_comment_active_aspx", PageIndex, recount, out ReCount, out PgCount, null);
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                info.Add("activeurl", Public.URLWrite(dr["ativeID"], "active"));
                ...
                info.Add("state", Public.GetEnumStateOp(dr["IsLock"], Convert.ToInt32(dr["id"]), uid, "activecomment"));
                info.Add("id", dr["id"]);
                infolist.Add(info);
            }
            dt.Dispose();
