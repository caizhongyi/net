...
namespace JuSNS.Web.system
{
    public class blog : ManagePage
    {
        ...
        protected void ShowInfo(ref NVelocity.VelocityContext context)
        {
            ...
            context.Put("cpagetitle", "博客管理 - 管理中心");
        ...
        protected void ShowList(ref VelocityContext context)
        {
            ...
            {
                dt = JuSNS.Home.UtilPage.GetPage("manager_blog_draft_aspx", PageIndex, recount, out ReCount, out PgCount, null);
            }
            else
            {
                if (!string.IsNullOrEmpty(kwd))
                {
                    ...
                    st[0].Blur = 3;
                    dt = JuSNS.Home.UtilPage.GetPage("manager_blog_key_aspx", PageIndex, recount, out ReCount, out PgCount, st);
                }
                else
                {
                    dt = JuSNS.Home.UtilPage.GetPage("manager_blogall_aspx", PageIndex, recount, out ReCount, out PgCount, null);
                }
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                info.Add("blogurl", Public.URLWrite(dr["id"], "blog"));
                ...
                if (Convert.ToInt32(dr["IsDraft"]) == 0)
                {
                    if (Convert.ToBoolean(dr["isrec"]))
                    {
                        info.Add("showrec", "<a href=\"javascript:;\" title=\"已推荐\" onclick=\"RecAll(" + dr["id"] + "," + uid + ",0,'blog')\" class=\"showrec1\"></a>");
                    }
                    else
                    {
                        info.Add("showrec", "<a href=\"javascript:;\" title=\"未推荐\" onclick=\"RecAll(" + dr["id"] + "," + uid + ",1,'blog')\" class=\"showrec\"></a>");
                    }
                }
                else
                {
                ...
                info.Add("id", dr["id"]);
                info.Add("state", Public.GetEnumStateOp(dr["IsLock"], Convert.ToInt32(dr["id"]), uid, "blog"));
