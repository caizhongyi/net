...
namespace JuSNS.Web.system
{
    public class multe : ManagePage
    {
        ...
        protected void ShowInfo(ref NVelocity.VelocityContext context)
        {
            ...
            context.Put("cpagetitle", "团购管理 - 管理中心");
        ...
        protected void ShowList(ref VelocityContext context)
        {
            ...
            if (!string.IsNullOrEmpty(kwd))
            {
                ...
                st[0].Blur = 3;
                dt = JuSNS.Home.UtilPage.GetPage("manager_multe_key_aspx", PageIndex, recount, out ReCount, out PgCount, st);
            }
            else
            {
                dt = JuSNS.Home.UtilPage.GetPage("manager_multeall_aspx", PageIndex, recount, out ReCount, out PgCount, null);
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    info.Add("showrec", "<a href=\"javascript:;\" title=\"已推荐\" onclick=\"RecAll(" + dr["id"] + "," + uid + ",0,'multe')\" class=\"showrec1\"></a>");
                }
                else
                {
                    info.Add("showrec", "<a href=\"javascript:;\" title=\"未推荐\" onclick=\"RecAll(" + dr["id"] + "," + uid + ",1,'multe')\" class=\"showrec\"></a>");
                }
                info.Add("state", Public.GetEnumStateOp(dr["IsLock"], Convert.ToInt32(dr["id"]), uid, "multe"));
                info.Add("id", dr["id"]);
                infolist.Add(info);
            }
            dt.Dispose();
