    ...
    public class album : ManagePage
    {
        ...
        protected void ShowList(ref VelocityContext context)
        {
            ...
            if (!string.IsNullOrEmpty(kwd))
            {
                ...
                st[0].Blur = 3;
                dt = JuSNS.Home.UtilPage.GetPage("manager_album_key_aspx", PageIndex, recount, out ReCount, out PgCount, st);
            }
            else
            {
                dt = JuSNS.Home.UtilPage.GetPage("manager_albumall_aspx", PageIndex, recount, out ReCount, out PgCount, null);
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                info.Add("posttime", Public.getTimeEXTSpan(Convert.ToDateTime(dr["CreateTime"])));
                ...
                {
                    info.Add("showrec", "<a href=\"javascript:;\" title=\"已推荐\" onclick=\"RecAll(" + dr["albumid"] + "," + uid + ",0,'album')\" class=\"showrec1\"></a>");
                }
                else
                {
                    info.Add("showrec", "<a href=\"javascript:;\" title=\"未推荐\" onclick=\"RecAll(" + dr["albumid"] + "," + uid + ",1,'album')\" class=\"showrec\"></a>");
                }
                info.Add("id", dr["albumid"]);
