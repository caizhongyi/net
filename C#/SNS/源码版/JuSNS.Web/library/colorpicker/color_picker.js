        ...
        jQuery('a').each(
			function() {
			    ...
			    if (relAttr.toLowerCase().indexOf('colorpicker') == 0) {
			        ...
			        var toobj;
			        var str = relAttr.substr(relAttr.indexOf("objshow=") + "objshow=".length);

			        var p = str.indexOf("&");
			        if (p != -1) {
			            toobj = str.substring(0, p);
			        }
			        else {
			            toobj = str;
			        }
			        var robj;
			        str = relAttr.substr(relAttr.indexOf("objcode=") + "objcode=".length);
			        p = str.indexOf("&");
			        if (p != -1) {
			            robj = str.substring(0, p);
			        }
			        else {
			            robj = str;
			        }
			        if (document.getElementById(toobj) && document.getElementById(robj)) {
			            try {
			                document.getElementById(toobj).style.backgroundColor = document.getElementById(robj).value;
			            } catch (e) { }
			        }
			    }
