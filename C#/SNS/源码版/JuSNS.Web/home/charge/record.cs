    ...
    public class record : UserPage
    {
        ...
        protected void ShowList(ref VelocityContext context)
        {
            ...
            if (Input.IsInteger(q))
            {
                ...
                dt = JuSNS.Home.UtilPage.GetPage("user_Charges_aspx", PageIndex, recount, out ReCount, out PgCount, st);
            }
            else
            {
                dt = JuSNS.Home.UtilPage.GetPage("user_Charge_aspx", PageIndex, recount, out ReCount, out PgCount, new SqlConditionInfo("@UserID", this.UserID, TypeCode.Int32));
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                rec.Add("succ", dr["IsSucces"].ToString().ToLower().Replace("true", "<span class=\"reshow\">成功</span>").Replace("false", "失败"));
