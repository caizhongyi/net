...
{
    public class UploadHead : UploadPic
    {
        public UploadHead(HttpFileCollection hfc)
            : base(hfc)
        {
        ...
        public override int Start()
        {
            ...
            Dictionary<string, PicConfigInfo> pci = PicConfig.Portrait;
            int j = 0;
            for (int i = 0; i < hfc.Count; i++)
            {
                ...
                if (err.IndexOf("$1") > -1)
                {
                    Thumbnail.MakeAllThumbnail(path, filename, 0);
                    _SuccessNum++;
                    issuc = true;
                }
                #endregion
