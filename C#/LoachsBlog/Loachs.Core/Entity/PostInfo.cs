    ...
    public class PostInfo : IComparable<PostInfo>
    {
        ...
        public string PageUrl
        {
            get
            {
                ...
                {
                    //case PostUrlFormat.Category:
                    //    url = string.Format("{0}post/{1}/{2}/page/{3}{4}", ConfigHelper.AppUrl, StringHelper.UrlEncode(this.Category.Slug), !string.IsNullOrEmpty(this.Slug) ? StringHelper.UrlEncode(this.Slug) : this.PostId.ToString(),"{0}", SettingManager.GetSetting().RewriteExtension);
                    //    break;
        ...
        public string FeedLink
        {
            get
            {
                return string.Format("<a href=\"{0}\" title=\"Subscribe to: {1} Comments\">RRS</a>", this.FeedUrl, this.Title);
            }
        }
