    ...
    public class Post : IPost
    {
        ...
        public int InsertPost(PostInfo postinfo)
        {
            ...
            string cmdText = @"insert into [loachs_posts]
                                (
                               [CategoryId],[Title],[Summary],[Content],[Slug],[UserId],[CommentStatus],[CommentCount],[ViewCount],[Tag],[UrlFormat],[Template],[Recommend],[Status],[TopStatus],[HideStatus],[CreateDate],[UpdateDate],[ShopLink],[ShopName]
                                )
                                values
                                (
                                @CategoryId,@Title,@Summary,@Content,@Slug,@UserId,@CommentStatus,@CommentCount,@ViewCount,@Tag,@UrlFormat,@Template,@Recommend,@Status,@TopStatus,@HideStatus,@CreateDate,@UpdateDate,@ShopLink,@ShopName
            ...
            OleDbParameter[] prams = { 
                                ...
                                OleDbHelper.MakeInParam("@Shoplink",OleDbType.VarChar,Int32.MaxValue,postinfo.Shoplink),
								OleDbHelper.MakeInParam("@Shopname",OleDbType.VarChar,Int32.MaxValue,postinfo.Shopname)
							};
        ...
        private List<PostInfo> DataReaderToCommentList(OleDbDataReader read)
        {
            ...
            while (read.Read())
            {
                ...
                postinfo.Shopname = Convert.ToString(read["Shopname"]);
                postinfo.Shoplink = Convert.ToString(read["Shoplink"]);
                list.Add(postinfo);
            }
            read.Close();
            return list;
