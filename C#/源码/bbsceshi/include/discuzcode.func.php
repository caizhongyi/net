...
/*
	...
	$Id: discuzcode.func.php 20866 2009-10-28 01:57:59Z monkey $
*/
...
function discuzcode($message, $smileyoff, $bbcodeoff, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '0', $pid = 0) {
	...
	if(!$bbcodeoff && $allowbbcode) {
		...
		if($allowmediacode && strpos($msglower, '[/flash]') !== FALSE) {
			$message = preg_replace("/\[flash\]\s*([^\[\<\r\n]+?)\s*\[\/flash\]/is", "<script type=\"text/javascript\" reload=\"1\">document.write(AC_FL_RunContent('width', '550', 'height', '400', 'allowNetworking', 'internal', 'allowScriptAccess', 'never', 'src', '\\1', 'quality', 'high', 'bgcolor', '#ffffff', 'wmode', 'transparent'));</script>", $message);
		}
...
function parsemedia($params, $url) {
	...
	if(in_array(count($params), array(3, 4))) {
		...
		switch($type) {
			...
			case 'flv':
			case 'swf':
				return '<script type="text/javascript" reload="1">document.write(AC_FL_RunContent(\'width\', \''.$width.'\', \'height\', \''.$height.'\', \'allowNetworking\', \'internal\', \'allowScriptAccess\', \'never\', \'src\', \''.$url.'\', \'quality\', \'high\', \'bgcolor\', \'#ffffff\', \'wmode\', \'transparent\'));</script>';
			case 'avi':
			case 'asf':
			case 'mpg':
			case 'mpeg':
...
function parseflv($url, $width, $height) {
	...
	if($flv) {
		return '<script type="text/javascript" reload="1">document.write(AC_FL_RunContent(\'width\', \''.$width.'\', \'height\', \''.$height.'\', \'allowNetworking\', \'internal\', \'allowScriptAccess\', \'never\', \'src\', \''.$flv.'\', \'quality\', \'high\', \'bgcolor\', \'#ffffff\', \'wmode\', \'transparent\'));</script>';
	} else {
		return FALSE;
	}
}
