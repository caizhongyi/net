...
document.write('.cMenuDivInner { margin:3 0 4 0 4; border-width: 1; border-style: solid; border-color: threedshadow threedhighlight threedhighlight threedshadow; }');
...
//this.width =  "auto";
//this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "宋体"; font-size: 13px;border:1px dashed #cccccc; margin:6 0 0 4;';
this.imgURL = 'images/';
this.debug  = 0;
...
//    ['linebreak'],
    ...
    ['HorizontalRule','Createlink','InsertTable','InsertImage','InsertFile','htmlmode','separator'],
...
this.btnList = {
    ...
    "subscript":      ['SubScript',            '下标',          'editor_action(this.id)',  'ed_format_sub.gif'],
    ...
    "insertfile":    ['InsertFile',          '插入文件(rar,mp3,flash等)',       'editor_action(this.id)',  'ed_file.gif'],
    "htmlmode":       ['HtmlMode',             '编辑HTML',   'editor_setmode(\''+objname+'\')', 'ed_html.gif']};
    //"popupeditor":    ['popupeditor',          '新窗口中编辑',     'editor_action(this.id)',  'fullscreen_maximize.gif'],
    //"about":          ['about',                '版权所有',  'editor_about(\''+objname+'\')',  'ed_about.gif'],
...
//
 ...
 // }
  ...
  // FontStyle (change CSS className)
    ...
    editdoc.execCommand('FontName',0,'DoNetBbs_FontStyle_2007');
    var fontArray = editdoc.all.tags("FONT");
    for (i=0; i<fontArray.length; i++) {
      if (fontArray[i].face == 'DoNetBbs_FontStyle_2007') {
        fontArray[i].face = "";
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(_editor_url + "/select_color.htm", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert table
      ...
      showModalDialog(_editor_url + "/insert_table.htm?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
    // insert image
	  ...
	  oPara.editdoc = editdoc;
	  oPara._editor_url = _editor_url;
	  oPara.window=window;
      showModalDialog(_editor_url + "insert_image.htm", oPara, "dialogWidth:400 px; dialogHeight:260 px;resizable: no; help: no; status: no; scroll: no; ");

    }


    // insert file
    else if (cmdID.toLowerCase() == 'insertfile'){
	  var oPara = new Object();
	  oPara.editdoc = editdoc;
	  oPara._editor_url = _editor_url;
	  oPara.window=window;
      showModalDialog(_editor_url + "insert_file.htm",oPara, "dialogWidth:400 px; dialogHeight:260 px;resizable: no; help: no; status: no; scroll: no; ");
    }
...
function editor_setmode(objname, mode) {
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  if (mode == "htmledit" || editor_obj.tagName.toLowerCase() == 'iframe') {
    config.mode = "htmledit";
    var editdoc = editor_obj.contentWindow.document;
  ...
  //
    ...
    html += '</style>\n'
      + '</head>\n'
      + '<body contenteditable="true" topmargin=1 leftmargin=1 title="按Ctrl+Enter或者Ctrl+s直接提交"  onkeydown="if ((event.ctrlKey)&&((event.keyCode==13)||(event.keyCode==83))){javascript:parent.JavaScrptWebSitePost();return false;}"  '
